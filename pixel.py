...
def pixel(filepath, size=(128, 128, 6), grid=0.20, sigma=1.5, demo=True):
    ...
    # find central metal atom coords in expanded cell and compress it to be
    # 2-dimensional
    ...
    for site in struct.sites:
        ...
        image[int(index_x)][int(index_y)][0] = site.z - min_z
        # atomic number
        element = Element(site.specie)
        image[int(index_x)][int(index_y)][1] = element.Z
        # image[int(index_x)][int(index_y)][1] = 14 if element.Z == 7 else element.Z
        # electronegativity
        image[int(index_x)][int(index_y)][2] = element.X
        # row
        image[int(index_x)][int(index_y)][3] = element.row
        # group
...
def data_augmentation(images):
    ...
    for name, image in images:
        ...
        # translate image
        crop_images = np.array([tf.image.crop_to_bounding_box(
            image, i, j, 64, 64).numpy() for i, j in vectors])
        # flip image
        flip_images = tf.image.flip_left_right(crop_images).numpy()
        # flip_images = tf.image.flip_up_down(crop_images)
        # resize image
        crop_images = tf.image.resize(
            images=crop_images, size=(
                32, 32), method='gaussian').numpy()
        flip_images = tf.image.resize(
            images=flip_images, size=(
                32, 32), method='gaussian').numpy()
...
def demo_pixels():
            ...
            for sub in substrates:
                ...
                for e in elements:
                    ...
                    images_origin.append(
                        (mesh + ' ' + add_N + ' ' + sub + ' ' + e, image))
    images_origin = np.array(images_origin)
    print(
        f"original shape: {images_origin.shape}, {images_origin[0][1].shape}")
    ...
    with open(os.path.join(root_dir, "demo_data/pixels.pkl"), 'wb') as f:
        # images is a list of tuple containing name `str` and pixel
        # `np.ndarray`
