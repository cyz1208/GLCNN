...
def wide_deep(
    Input_1, Input_2, kernel_nums: tuple = (
        6, 16, 120), kernel_sizes: tuple = (
            5, 5, 5), fc_sizes: tuple = (
                2000, 200, 1), dropout_rate: float = 0.2, activation: str = "relu"):
    """
    GLCNN architecture.
    ...
    assert len(kernel_nums) == len(
        kernel_sizes), "kernel_nums and kernel_sizes should have same length"

    x_1 = Lenet5(Input_1, kernel_nums=kernel_nums, kernel_sizes=kernel_sizes,
...
def Lenet5(Input, kernel_nums, kernel_sizes, dropout_rate, activation):
    """
    CNN part.
