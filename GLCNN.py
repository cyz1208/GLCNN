...
def timer(func):
    ...
    def inner(*args, **kargs):
        ...
        print(
            f"func {func.__name__} consumed time: {np.round(t1 - t0, decimals=3)} sec.")
        return res
    return inner
...
def data_process(repeat: int, demo: bool, channels: Iterable):
    ...
    train_index, test_index = train_test_split(
        index, test_size=0.2, random_state=None, shuffle=True)
    val_index, test_index = train_test_split(
        test_index, test_size=0.5, random_state=None, shuffle=True)
...
def scheduler(epoch, lr):
    ...
    if epoch < warmup_steps:
        ...
        return 0.0001 + 0.0009 * (epoch + 1) / \
            warmup_steps  # Linear increase warmup
    elif epoch == warmup_steps:
        return 0.001
    elif epoch > warmup_steps:
        # return max(lr * tf.math.exp(-0.10), 0.0001) if epoch % 5 == 0 else lr
        # # Stepped exponential reduction
        # Gradual exponential reduction
        return max(lr * tf.math.exp(-0.015), 0.0001)
    else:
...
def train_model(
    ...
    model.fit(X_train, y_train,
              ...
              callbacks=[
                  tf.keras.callbacks.TensorBoard(
                      log_dir=log_dir, histogram_freq=5),
                  # tf.keras.callbacks.EarlyStopping(monitor="val_loss", patience=10, verbose=2),
                  tf.keras.callbacks.ModelCheckpoint(
                      model_ckpt, save_best_only=True, verbose=2, monitor="val_mae"),
    ...
    print(
        f"model train time consumed: {np.round(end - start, decimals=3)} sec.")
...
def test_model(X_test, y_test, y_origin, model_ckpt, repeat):
    ...
    # each set contains predicted value of DA samples from same original one
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="DA iterations with maximum of 20")
    parser.add_argument(
        "-e",
        "--epoch",
        type=int,
        default=200,
        help="epoch of model training")
    ...
    model = build_model(X_test_1.shape[1:],
                        X_test_2.shape[1:],
                        kernel_nums=KERNEL_NUMS,
                        kernel_sizes=KERNEL_SIZES,
                        fc_sizes=FC_SIZES,
